...
let calculate_distance left right =
  ...
  for i = 0 to List.length sorted_left - 1 do
    ...
    distances := Int.abs distance :: !distances
  done;
  distances
...
let () =
  ...
  let distances = calculate_distance left right in
  let result = ref 0 in
  for i = 0 to List.length !distances - 1 do
    result := !result + List.nth !distances i
  done;
